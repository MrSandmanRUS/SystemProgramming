  ...
  union                                           /*определить об'единение  */
   {
    ...
    {
        unsigned char OP;                             /*Кодоперации            */
        unsigned char L1L2;                           /*L1 и L2                 */
        short B1D1;                                   /*B1 и D1                 */
        short B2D2;                                   /*B2 и D2                 */
    };

    union                                           /*определитьоб'единение  */
    {
        unsigned char BUF_OP_SS [6];                  /*оределитьбуфер         */
    ...
    if                                            /* если псевдоопераци€ DC */
     ...
     )                                            /* следующее:             */
     {
      ...
      PRNMET = 'N';                               /*  занулить PRNMET зн.'N'*/
        ...
        T_SYM[ITSYM].DLSYM = lenght;
        T_SYM[ITSYM].PRPER = 'R';
        if ( CHADR % lenght ) {
            CHADR = (CHADR /lenght + 1) * lenght;
            T_SYM[ITSYM].ZNSYM = CHADR;
        }
        PRNMET = 'N';
        //CHADR = CHADR + atoi(&len);
    }
    else
...
int SUSING()                                      /*подпр.обр.пс.опер.USING */
 {
  ...
  if ( isalpha ( (int) *METKA2 ) || ((int) *METKA2) == 64)                /*если лексема начинаетс€ */
   {                                              /*с буквы, то:            */
...
int SRR()                                         /*подпр.обр.опер.RR-форм. */
 {
  ...
  if ( isalpha ( (int) *METKA1 ) || ((int) *METKA1) == 64)                /*если лексема начинаетс€ */
   {                                              /*с буквы, то:            */
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
						  /* со знач.перем. *METKA1 */
...
int SSS()                                         /*ïîäïð.îáð.îïåð.SS-ôîðì. */
{
    ...
    const char *search_characters = "+";
    char *mystring = (char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND;
    char *c = mystring;
    int flag = 0;
    while (*c)
    {
        if (strchr(search_characters, *c))
        {
            flag = 1;
        }
    ...
    else if (3 == sscanf ((char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "%6[^+](%d),%8s", op1, &length, op2)) {
        offset = 0;
        op1[6] = ' ';
        op1[7] = ' ';
        printf ("%s %s %d %d\n", op1, op2, offset, length);
